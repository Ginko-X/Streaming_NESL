...
\def\eqnumthree#1{eq-lem24-c3-{#1}}
 ...
 \def\sgmbpp#1{\sgm_{#1}[\s_1 \|-> \sgm_{#1}''(\s_1),...,\s_j \|-> \sgm_{#1}''(\s_j)]} 
    ...
    By IH on $\MP_1'$, $\MP_2'$ with \ref{eq-lem24-c3-6}, we get a derivation $\MP_0$ of 
   ...
   \eq{\eqnumthree{34}}
    ...
    \end{split}}   
   	...
   	By IH on \ref{\eqnumthree{33}} with $\MP_1''$, $\MP_2''$, \ref{eq-lem24-c3-4}, we obtain 
   	a derivation of 
   	$$\seval{p'}{\sgmbpp{1} \x{\S} \sgmbpp{2}}{\cc}{\sgmpx},$$
   	
   	and replacing the start store with the right-hand side of \ref{\eqnumthree{34}} gives us a derivation $\MP'$ of 
   	  $$\seval{p'}{\sgmx[\s_1 \|-> \sgm_1''(\s_1) {\++} \sgm_2''(\s_1),...,\s_j \|-> \sgm_1''(\s_j) {\++} \sgm_2''(\s_j)]} \cc \sgmpx $$
   	
    With \ref{\eqnumthree{32}}, we can replace all the new bound values  $\sgm_1''\x{\S} \sgm_2''(s_i)$ in $\MP'$ with $\sgm_1''(\s_i) {\++} \sgm_2''(\s_i)$ for all $i \in \{1,...,j\}$, giving us another derivation $\MP''$ of 
    
    $$\seval{p'}{\sgmx[\s_1 \|-> \sgm_1''\x{\S} \sgm_2''(s_1),...,\s_j \|-> \sgm_1''\x{\S} \sgm_2''(s_j)]} \cc \sgmpx $$
   	
   	Since $\ref{\eqnumthree{30}}$ and $\ref{\eqnumthree{31}}$, we know $\sgmx(\s_c)$ = $\c_1' {\++}  \c_2' = \<()|...\'>$,
