	...
	\RiLa{
		\left(
			\begin{aligned}
				...
				&??? \forall j \in \{1,...,i\}. \s_j \in \Sout \wedge \s_j > \s_c
			\end{aligned}
		\right)}
	\UC{\seval{\withctrl{\s_c}{p}{\Sin}{\Sout}}{\sgm}{\c}{\sgm[\s_1 \|-> \emptyv, ..., \s_i \|-> \emptyv]}}
}}\\[2ex]

\makebox[1.0\textwidth][c] {\PRule{Wc-Nonemp}{\PT{
	\AC{\seval{p}{\sgm}{\c_1}{\sgm''}}
	\RiLa{\left(
		\begin{aligned} 
		&\sgm(\s_c)= \c_1 = \< () | \c_2 \'> \\
	    &??? \forall j \in \{1,...,i\}. \s_j \in \Sout \wedge \s_j > \s_c
		\end{aligned}\right)}
	\UC{\seval{\withctrl {s_c} p \Sin \Sout } \sgm {\c} \sgm[\s_1 \|-> \sgm''(\s_1),...,\s_i \|-> \sgm''(\s_i)]}
}}}\\[2ex]
...
\begin{lem} [\textbf{Stores concatenation lemma}] \label{lem-sgm-join}
	...
	and $\FV{p}{\c_1} \cap \S = \emptyset $, $\FV{p}{\c_2} \cap \S = \emptyset $
...
\PT{
	...
	\RiLa{\left(
		   ...
		   \end{aligned}
		 \right)}
	\UC{\Trans{\del}{\Comp{e}{x}{y}{\cdot}}{\s_0}{\s_1}{\sfun{\sdef{\s_0}{\usum(\s_2)}; \withctrl{\s_0}{p}{\Sin}{\overline{\st}}}{(\st,\s_2)}}}
}
