...
%translation
\def\sfun#1#2{(#1,#2)}
\def\Transf#1#2#3#4#5{\Trans{}{#1(#2)}{#3}{#4}{#5}}
...
Draft version \versionnumber: added the proof of the main correctness theorem and part of the proof of the built-in functions correctness theorem
(page 6-12), and related definitions and lemmas.
...
\begin{lem} \label{lem-sgm-join}
	...
	then $\exists \st'.\seval{p}{\sgm_1 \x{\st} \sgm_2}{\a_1 \++ \a_2}{\sgm_1' \x{\st'} \sgm_2'}$
\end{lem}
...
\PT{
	\Axiom{\Transf{\constn{n}}{}{\s_0+1}{\s_0}
...
\begin{lem}
	...
	\item $\Eval{}{\EvalF{\Tupk{v}}}{v}$ (by $\ME$)
...
\def\pconst{\sdef{\s_0}{\constaf{n}}}
	...
	Since $\constaf{n}$ takes no arguments, there is only one 
	possibility for $\MP$:
	$$\PT{
	...
	Then we have $\MV$ = $\PT{\Axiom{\ValRep{n}{\int}{\sgm'(\s_0)}}}$. \\
...
\def\pplus{\sdef{\s_0}{\maptwof{+}{\s_1}{\s_2}}}	
	So $k=2,\tau_1 = \tau_2 = \tau = \int, v_1= n_1, v_2= n_2, v = n_3,
	 ...
	 $\ValRep{n_1}{\int}{\sgm(\s_1)}$ and 
	 $\ValRep{n_2}{\int}{\sgm(\s_2)}$, which implies
	 $\sgm(\s_1) = \singl{n_1}$ and $\sgm(\s_2) = \singl{n_2}$. \\
	 
	 For (v) we have $\s_1 < \s_0$ and $\s_2 < \s_0$. \\
	 
	 $\MP$ must have the shape: 
	 $$\PT{
		\UCN{\MP_1}{\blockf{\maptwo{+}}{\a_1,\a_2}{\a}}
		\Axiom{\sevalfg{\maptwo{+}}{\a_1',\a_2'}{\emptyv}{\emptyv}}
		\RiLa{(\a_1 {\++} \a_1' = \singl{n_1},\a_2 {\++} \a_2' = \singl{n_2})}
		\BC{\sevalfg{\maptwo{+}}{\singl{n_1},\singl{n_2}}{\vunit}{\a}}
		\RiLa{(\sgm(\s_1)= \singl{n_1}, \sgm(\s_2) = \singl{n_2})}
		\UC{\seval{\pplus}{\sgm}{\vunit}{\sgm[\s_0 \|-> \a]}}
	 }$$  
	
	Since there is only one rule for $\MP_1$, by which $\a_1$ must be
	$\singl{n_1}$ and $\a_2$ must be $\singl{n_2}$ (, and $\a_1'$ = $\a_2' = \emptyv$), that is, 
	$$\MP_1 = \PT{\Axiom{\blockf{\maptwo{+}}{\singl{n_1}, \singl{n_2}}{\singl{n_3}}}}$$
	
	Therefore, $\sgm' = \sgm[\s_0 \|-> \singl{n_3}]$.\\
	Now it is clear that
	$\MV = \infer{\ValRep{n_3}{\int}{\sgm'(\s_0)}}{}$,
...
\begin{thm}
	...
	\begin{enumerate}[(i)]
		...
		\item \label{mainthm-assum-env} $\forall x \in dom(\Gam). \Type{}{\rho(x)}{\Gam(x) } \wedge \sids{\del(x)} \SetLe \s_0  \wedge  \ValRep{\rho(x)}{\Gam(x)}{\sgm(\del(x))}$ \\
	\textbf{then} 
...
\def\sgmsz{\sgm[\s_0 \|-> \kunit]}
		...
		\PT{
			...
			\UC{\Trans{\del}{\Comp{e_1}{x}{y}{\cdot}}{\s_0}{\s_1}
				{\sfun{\sdef{\s_0}{\usum(\s_2)}; \withctrl{\s_0}{p_1}{\st_2}}{(\st_2,\s_2)}}}
		}
...
\def\usumdef{\sdef{\s_0}{\usum(\s_2)}} 
	...
	From the rules $\PName{UsumF}$ and $\PName{UsumT}$ we know that $\b_1$ must end with (and include exactly) one $\T$ so that $\MP_0'$ can terminate. 
...
% case let-binding 
 ...
 \begin{enumerate}[(a)]
 	...
 	\item $\MV_1$ of $\ValRep{v_1}{\tau_1}{\sgm_1(\st_1)}$
 ...
 \begin{enumerate}	[(a)]
 	...
 	\item $\MV_2$ of $ \ValRep{\sgm_2}{\tau}{\sgm_2(\st)}$
...
\end{enumerate}
 ...
 Take $\sgm' = \sgm_2$ (thus $\MV$ = $\MV_2$)  and we are done. 
