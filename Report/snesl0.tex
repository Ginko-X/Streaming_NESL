...
% IH --------	
  Now it remains to build $\MP_{j+1}$. \\
	...
	\eq{\eqnum{34}} {\sgm_1' \~\S \sgm_2' \~\S ... \~\S \sgm_k' \~\S \sgm_j}
	\eq{\eqnum{32}}
		{\k{\x{\S}\sgm_i'} = \sgm_j}
	
	Also note that we now have 
		\eq{\eqnum{33}}{\Sin = \FV{p_1} \subseteq ({\st_1} \cup \{\s_1,...,\s_j\}) \cap \S = \emptyset} 
...
\def\sgmp-nonempty{\sgm[\s_0 \|-> \kunit, \s_1' \|-> \sgm''(\s'_1),...,
	...
	\eq{p1-sgms-sgms'}{
		\seval{p_1}{(\x{\S} \sgm_i')^k_{i=1}}{\kunit}{(\x{\S} \sgm''_i)^k_{i=1}}
	}

    Let $\sgm'' = (\x{\S} \sgm''_i)^k_{i=1}$. Also with $\eqref{\eqnum{32}}$, we replace both the start and ending stores in $\eqref{p1-sgms-sgms'}$, giving us 
    a derivation $\MP_{j+1}'$
    $$\seval{p_1}{\sgm_j}{\kunit}{\sgm''}$$
    
    And for $\forall i \in \{1,...,l\}$, by Definition \ref{def-sgm-join} we have
    $$\sgm''(\s_{j+i}) = \sgm''_1(\s_{j+i}) {\++} ... {\++} \sgm''_k(\s_{j+i})$$
	
    
	
    Now we can build $\MP_{j+1}$ using the rule $\PName{Wc-Nonemp}$ as follows:
	$$\PT{
    ...
    So in this subcase $\sgm' = \sgm_j[\l{\s_{j+i}\|-> \sgm''(\s_{j+i})}]$.\\
    
	TS : (vi)  \\
	Let $\sgm'(\st_2) = w'$, and $\sgm''_i(\st_2) = w'_i$ then it is easy to show that 
...
%vii
\item TS: (vii) \\
It is clear that in both subcase we have $$dom(\sgm') = dom(\sgm) \cup \{\s_0\} \cup \{\s_1,...,\s_j\} \cup \{\s_{j+1},...,\s_{j+l}\}$$
Since $\forall \s \in \{\s_0\} \cup \{\s_1,...,\s_j\} \cup \{\s_{j+1},...,\s_{j+l}\}. \s_0 \le \s$, 
therefore $\sgm' \ConEq{\s_0} \sgm$.
