...
\def\sevalf#1#2#3#4{SEva_{\lcall}(#1,...,#2) \Eva^{#3} #4}
\def\block#1#2#3{Block_{\lcall}(#1,...,#2) \Downarrow #3}  
\def\unary#1#2#3{Unary_{\lcall}(#1,...,#2) \Downarrow #3}
\def\da{\Downarrow}

\def\blockv#1#2#3#4{Block_{\lcall,#4}(#1,...,#2) \Downarrow #3}  
\def\unaryv#1#2#3#4#5{Unary_{\lcall,#4}(#1,...,#2) \Downarrow^{#5} #3}
...
%translation
\def\sfun#1#2{(#1,#2)}
\def\tranf#1#2#3#4#5{\Env Trans_{\hcall}(#1,...,#2) \xRightarrow[#4]{#3} #5}  
...
\PT{\AC{\sevalf{\a_1}{\a_k}{\a_c}{\a}}
	...
	\UC{\seval{\sdef{\s}{\lcall\Tupk\s}}{\sgm}{\a_c}{\sgm[\Map{\s}{\a}]}}
} \\[2ex]
