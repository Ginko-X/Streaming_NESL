...
\def\sgmp-nonempty{\sgm[\s_0 \|-> \kunit, \s_1' \|-> \sgm''(\s'_1),...,
	...
	Then by Theorem \ref{thm-svcode-determ}, we get
	\eq{sgms'-join}{
		(\x{\st_2} \sgm'_i)^k_{i=1}= \sgm''}
...
%vii
	From \eqref{sgms-join} we have $\sgm_i \~{\st_1} \sgmsz$, and  
	from \eqref{sgms'-join} $\sgm'_i \~{\st_2} \sgm''.$ \\
	\eqref{compIH3} implies $$(\sgm'_i \ConEq{\s_0} \sgm_i)^k_{i=1}$$.
