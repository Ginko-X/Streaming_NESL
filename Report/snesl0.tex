...
\def\sgmsz{\sgm[\s_0 \|-> \vrange{()_1}{()_k}]}
...
\def\sgmp-nonempty{\sgm[\s_0 \|-> \kunit, \s_1' \|-> \sgm''(\s'_1),...,
	...
	$$\seval{p_1}{(\x{\st_1} \sgm_i)^k_{i=1}}{\kunit}{(\x{\st_1} \sgm'_i)^k_{i=1}}$$
	
	%TODO add property lemma
	By Lemma \ref{lem-join1} we can obtain
	\eq{sgms-join}{
		(\x{\st_1} \sgm_i)^k_{i=1} = \sgmsz}
	
	and \eq{sgms'-join}{
		(\x{\st_1} \sgm'_i)^k_{i=1}= \sgm''}
	
	in which $\sgm''(\st_2) = \sgm'_1(\st_2) {\++} ... {\++} \sgm'_k(\st_2).$\\
	Let $\sgm'_i(\st_2) = w'_i$ and  $\sgm''(\st_2) = w'$, then
	$w' = w'_1 {\++} ... {\++} w'_k$. \\
	
	Since $\sgm'(\st_2) = \sgm''(\st_2) = w$, and $\sgm'(\s_2) = \sgm(\s_2) = \stwo,$
	therefore $\sgm'((\st_2,\s_2)) = (\sgm'(\st_2),\sgm'(\s_2)) = (w',\stwo)$, 
...
%vii
	From \eqref{sgms-join} we have $\sgm_1 \~{\st_1} \sgmsz$ and 
	$\sgm'_1 \~{\st_2} \sgm''.$ 
	Take $i=1$ in \eqref{compIH3}, we have $\sgm'_1 \ConEq{\s_0+1} \sgm_1$, 
	hence $\sgm'_1 \ConEq{\s_0} \sgm_1$. 
	Using Lemma \ref{lem-join2} twice, we obtain 
	$$\sgm'' \ConEq{\s_0} \sgmsz.$$
	Therefore, $\sgmszs \ConEq{\s_0} \sgmsz \ConEq{\s_0} \sgm$.

	\end{itemize}

% viii
	\item TS: $\s_0 \le \s_1$ \\
	From \eqref{compIH4} we immediately get $\s_0 \le \s_1$.
