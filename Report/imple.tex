...
If there is no free variables in $e_1$ (i.e., the list after $\*{using}$ is empty), then the next step of translating a general comprehension will be translating the comprehension body $e_1$ in the new symbol table $[x \|->\st_0]$, and the translation is finished.

For the case where $e_1$ uses some free variables $x_1,...,x_k$, we need to generate new streams, each of which is a $l$-wise replicate of the stream of $x_1,...,x_k$ respectively. 
