...
and $\Sigma$ from the function identities to their types: 
...
We use $\<a_1,...,a_k\'>$ to denote a primitive stream of $k$ elements $a_1,...,a_k$. Thus,
$$ \a ::= \<a_1,...,a_k\'>$$
$$ w ::= \a \ | \ (w_1,w_2) $$
...
text-processing tasks and near-linear scales on multi cores.
...
An issue here for Xducers is dealing with runtime errors. 
...
So the translation will look like:\\[3ex]
	\PT{
		\AC{\Trans{\del}{e_1}{}{}{(p_1, (st_0,s_b))}}
		\AC{\Trans{[x \|-> {\st_0}, \j{x_i \|-> \st'_i}]}{e}{s_1}{}{\sfun{p_2}{\st}}}
		\RiLa{(\j{\del(x_i) = \ \st_i})}		
...
Now we use a structure $process$, a tuple of four components including a Xducer, to stand for one node on the streaming DAG:
